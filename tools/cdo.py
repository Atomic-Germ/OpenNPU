...
PM_BASE  = 0x10000   # program memory (within tile local space)
PM_END   = 0x20000
DM_BASE  = 0x20000   # data memory
DM_END   = 0x30000
...
def area_name(row: int, off: int) -> str:
    if row == SHIM_ROW:
        return "SHIM_DMA" if SHIM_DMA_BASE <= off < SHIM_DMA_BASE + 0x1000 else "SHIM"
    if row == MEM_ROW:
        return "MEM_DM" if DM_BASE <= off < DM_END else "MEM_REG"
    if CORE_ROW_MIN <= row <= CORE_ROW_MAX:
        if PM_BASE <= off < PM_END:  return "CORE_PM"
        if DM_BASE <= off < DM_END:  return "CORE_DM"
        return "CORE_REG"
    return f"ROW{row}"
...
class CdoDmaWriteCmd(CdoCmd):
    ...
    def is_pm_load(self) -> bool:
        return CORE_ROW_MIN <= self.row <= CORE_ROW_MAX and PM_BASE <= self.local_off < PM_END

    @property
    def is_dm_init(self) -> bool:
        return DM_BASE <= self.local_off < DM_END

    def describe(self) -> str:
        a = area_name(self.row, self.local_off)
        note = " [PM_LOAD]" if self.is_pm_load else (" [DM_INIT]" if self.is_dm_init else "")
...
class CdoParser:
    ...
    def extract_tile_programs(self) -> list:
        """Collect all DMAWRITE PM loads and assemble per-tile VLIW programs."""
        pm_segs: dict = {}
    ...
    def topology_summary(self) -> str:
        ...
        for cmd in self.cmds:
            ...
            if isinstance(cmd, (CdoWriteCmd, CdoMaskWriteCmd)):
                if cmd.row == SHIM_ROW and SHIM_DMA_BASE <= cmd.local_off < SHIM_DMA_BASE + 0x1000:
                    shim_dma += 1
        ...
        for (col, row), nb in sorted(pm_bytes.items()):
            lines.append(f"    tile({col},{row})  {nb} bytes  ({nb//16} VLIW instrs)")
        if not pm_bytes:
